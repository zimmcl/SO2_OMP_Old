...
cantidad_datos(void);
void
almacenar_datos(struct Pulso *pulsos_vertical[], struct Pulso *pulsos_horizontal[], int);
void
...
void
    gate_Pulsos(int cant_datos, struct Pulso *pulsos_M[],
        struct Pulso *gate_Estruc[][((int) (0.5 * cant_datos) / 250) + 1], int);
float
get_autocorrelacion(int cant_datos, struct Pulso *gate_Estruc[][((int) (0.5 * cant_datos) / 250) + 1], int);
...
{
  int iThreads = 1;
  double inicio, final; //Variables para cálculo de tiempo
  ...
  almacenar_datos(pulsos_vertical, pulsos_horizontal, iThreads);
  ...
  gate_Pulsos(cant_datos, pulsos_vertical, gate_Vertical, iThreads);
  gate_Pulsos(cant_datos, pulsos_horizontal, gate_Horizontal, iThreads);
  //guardar_gate_pulsos(cant_datos, gate_Vertical, gate_Horizontal);
  autocorrelacion_v = get_autocorrelacion(cant_datos, gate_Vertical, iThreads);
  autocorrelacion_h = get_autocorrelacion(cant_datos, gate_Horizontal, iThreads);
  printf("---------------------\n");
  /**
 ...
 */
void
almacenar_datos(struct Pulso *pulsos_vertical[], struct Pulso *pulsos_horizontal[], int iThreads)
{
  source = fopen("../pulsos.iq", "rb");
  if (source)
    {
      ...
      //#pragma omp parallel for shared(pulso) private(i,j) num_threads(iThreads)
      for (i = 0; i <= 2 * cant_datos; i++)
        {
          pulso = malloc(sizeof(struct Pulso));
          n = fread(&dato, sizeof(float), 1, source);
 ...
 */
void
gate_Pulsos(int cant_datos, struct Pulso *pulsos_M[], struct Pulso *gate_Estruc[][((int) (0.5 * cant_datos) / 250) + 1], int iThreads)
{
  int gate = (0.5 * cant_datos) / 250;
  int aux = 0, j = 0, i;
  float real = 0, img = 0;
  ...
  //#pragma omp parallel for private(i,j) num_threads(iThreads)
 ...
 */
float
get_autocorrelacion(int cant_datos, struct Pulso *gate_Estruc[][((int) (0.5 * cant_datos) / 250) + 1], int iThreads)
{
  int gate = (0.5 * cant_datos) / 250;
  int i;
  float v_resultado = 0;
  ...
  #pragma omp parallel for shared(v_resultado,gate_Estruc) private(i)
